(function(){
  window.evaluate=function(h,mutate=true){
    ...
    if(p<0){
      ...
      if(!h.downActive){
        if(h.prevPct===null||h.prevPct===undefined){
          if(p<=-r.downPct+0.0001){buy=true;msg=`−${r.downPct}%到達`;if(r.downContinue){h.downActive=true;h.lastDownPurchasePct=-r.downPct}}
        }else if(h.prevPct>-r.downPct+0.0001&&p<=-r.downPct+0.0001){
          buy=true;msg=`−${r.downPct}%到達`;if(r.downContinue){h.downActive=true;h.lastDownPurchasePct=-r.downPct}
        }
      }else if(r.downContinue){
        const next=h.lastDownPurchasePct-r.downContinuePct;
        if((h.prevPct===null||h.prevPct>next)&&p<=next+0.0001){buy=true;msg=`−${r.downContinuePct}%継続購入`;h.lastDownPurchasePct=next}
      }
    }else{
      ...
      if(!h.upActive){if((h.prevPct===null||h.prevPct<r.upPct)&&p>=r.upPct-0.0001){buy=true;msg=`＋${r.upPct}%到達`;if(r.continue){h.upActive=true;h.lastUpPurchasePct=r.upPct}}}
      else if(r.continue){const next=h.lastUpPurchasePct+r.continuePct;if((h.prevPct===null||h.prevPct<next)&&p>=next-0.0001){buy=true;msg=`＋${r.continuePct}%継続購入`;h.lastUpPurchasePct=next}}
    }
  ...
  window.kobotradeResetHoldingJudgment=function(i){
    const h=state.holdings[i]; if(!h)return false;
    h.upActive=false;h.lastUpPurchasePct=null;h.downActive=false;h.lastDownPurchasePct=null;h.prevPct=null;h.lastDecisionPct=null;h.lastDecisionBuy=false;h.lastDecisionMsg="";save();return true;
  };
})();
